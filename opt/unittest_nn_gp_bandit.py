...
def get_tp_comp(nn_type):
  ...
  tp_comp = get_transportnndistance_from_args(nn_type, 1.0)
  return tp_comp

class NNGPBanditTestCase(BaseTestClass):
  ...
  def test_instantiation(self):
    ...
    optimiser = nn_gp_bandit.NNGPBandit(func_caller,
                                        worker_manager, tp_comp, reporter='silent')
  ...
  def _get_optimiser_args(self, nn_type):
    ...
    return get_optimiser_args(self, nn_type, nn_gp_bandit.all_nn_gp_bandit_args)
  ...
  def test_gpb_optimisation_single(self):
    """ Tests optimisation of a single point. """
    self.report('Testing NNGPBandit with a single worker.')
    worker_manager = SyntheticWorkerManager(1, time_distro='const')
    func_caller = FunctionCaller(cnn_syn_func1, self.cnn_domain)
    tp_comp = get_tp_comp('cnn')
    options, options_clone, reporter, _, _ = self._get_optimiser_args('cnn')
    opt_val, opt_pt, history = nn_gp_bandit.nngpb_from_func_caller(
  ...
  def test_gpb_optimisation_asynchronous(self):
    """ Tests optimisation of a single point. """
    self.report('Testing NNGPBandit with 4 asynchronous workers.')
  ...
  def test_gpb_optimisation_synchronous(self):
    """ Tests optimisation of a single point. """
    self.report('Testing NNGPBandit with 4 synchronous workers.')
    worker_manager = SyntheticWorkerManager(4, time_distro='halfnormal')
    func_caller = FunctionCaller(cnn_syn_func1, self.cnn_domain)
    tp_comp = get_tp_comp('cnn')
    options, options_clone, reporter, _, _ = self._get_optimiser_args('cnn')
